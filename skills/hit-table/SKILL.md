---
name: hit-table
description: "Используй этот skill для получения информации об источнике (таблице) hits, который используется для сработок антифрода. "
---
...
- пользователь спрашивает про сработки
- нужны `policy_action`, `main_rule`, резолюции, save/fp, жалобы;

одна строка = одна сработка антифрод-мониторинга. Это не полная транзакционная история клиента.
