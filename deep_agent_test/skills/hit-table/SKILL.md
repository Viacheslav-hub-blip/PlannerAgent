...
- `event_dt` — дата сработки в формате `YYYYMMDD` (например, `20260124`). **Предпочтительное поле для фильтрации и связи с raw-таблицами.**
- `event_time` — дата-время сработки в читаемом формате `YYYY-MM-DD HH:MM:SS` (например, `2026-01-24 17:39:03`). Используй для отображения и контекста; **не копируй это значение как фильтр `event_time` в `uko_event`**, там другой формат хранения.
...
## Связывание с raw-таблицами по дате

При переходе из hits в `cards_event` или `uko_event`:

1. Сначала ищи по `event_id`, если он есть в hits.
2. Для дневного сопоставления и fallback-поиска используй **`event_dt`**, а не точное `event_time`.
3. Значение `event_dt` из hits передавай в raw-таблицы как `YYYYMMDD` без преобразования в ISO (`20260124`, не `2026-01-24`).
4. Не добавляй фильтр `event_time = <значение из hits>`, если целевая таблица — `uko_event`: там `event_time` хранится в другом формате.
5. Если нужна точность по времени только в `uko_event`, используй `event_dttm_readable`, а не `event_time`.
