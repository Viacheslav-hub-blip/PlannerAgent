...
---
Возвращает краткую структуру внутренних файлов тебя как агента
Используй tool, когда нужно понять расположение модулей (тебя) агента для саморедактирования 
    ...
    *,
    ...
    return "\n".join(
        [
            "# Структура агента",
            "",
            f"- Корень workspace: `{workspace_path}`",
            f"- Внутренние файлы агента: `{workspace_tool_path(agent_path, workspace_root, directory=True)}`",
            f"- Директория skills: `{workspace_tool_path(skills_path, workspace_root, directory=True)}`",
            f"- Максимум строк дерева: `{max_tree_entries}`",
            "",
            "## Внутренние файлы",
            "",
            "```text",
