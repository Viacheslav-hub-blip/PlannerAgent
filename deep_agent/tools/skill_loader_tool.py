...
from deep_agent.agent_settings import AgentSettings, workspace_tool_path
from deep_agent.middleware.skills_context_middleware import (
...
Загружает содержимое выбранных skills (`SKILL.md`) одним вызовом.

Используй когда:
- нужного skill нет в блоке предварительно загруженных skills;
- загруженный skill явно ссылается на другой известный путь skill;
- supervisor уже знает точное имя skill или виртуальный путь из проверенного контекста.

Не используй когда:
- нужен `fields.md`, `joins.md` или другой вспомогательный файл. `load_skills` загружает только `SKILL.md` из
  индекса skills. Для вспомогательных файлов используй filesystem tools.

Что делает:
- читает каждый запрошенный `SKILL.md` и возвращает его содержимое без пересказа;
- добавляет перед каждым загруженным skill его имя и workspace-путь;
- пропускает skills, которые уже загружены через middleware preload, прошлые вызовы `load_skills` или `already_loaded`.

Аргументы:
- `skill_names`: известные имена skills или workspace-пути через запятую в одной строке, например
  `skill-a, skill-b` or
  `/skills/skill-a/SKILL.md, /skills/skill-b/SKILL.md`;
- `already_loaded`: skills через запятую, которые не нужно загружать повторно. Может быть пустым.

Используй `load_skills` только для пакетной загрузки проверенных skills в контекст supervisor. Не угадывай имена и
не запрашивай все доступные skills. Не передавай пути вроде `/skills/name/fields.md`; это не skills.
""".strip()
...
def _build_report(
    ...
    if unknown:
        notes.append("Не найдены в индексе skills: " + ", ".join(unknown))
    if notes:
        sections.append("\n".join(notes))
    return "\n\n".join(sections)
