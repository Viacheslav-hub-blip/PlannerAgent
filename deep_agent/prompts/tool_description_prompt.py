...
---
Описание:
Запускает одного subagent и возвращает его финальный отчет.
...
- `description`: задача subagent с входными данными, ограничениями, ожидаемым результатом и условием остановки.

Выход:
- текстовый отчет subagent о выполненной работе, найденных фактах, созданных artifacts, проверках и ограничениях.

Используй когда:
- изолированная задача должна быть выполнена в отдельном контексте;
- результат subagent нужен как компактный отчет;
...
Не используй когда:
- следующий шаг — один маленький прямой tool call вне исследования или цепочки изменений code/workspace;
- у текущего агента уже достаточно проверенных доказательств для ответа;
- делегирование только повторит уже известный контекст.
...
task(
  ...
  description="
Цель: исправить некорректную обработку workspace-путей.
Границы: /deep_agent/execution/python_sandbox.py и тесты.
Ограничения: без широкого переписывания, сохранить существующий API кроме документированного контракта python tool.
Ожидаемый отчет: измененные файлы, выполненные команды, результаты тестов, ограничения.
Условие остановки: focused tests прошли или найден blocker с доказательствами.
"
)
```
""".strip()
...
Выход:
- результат записи и verification notice или сообщение об ошибке, если файл нельзя прочитать после записи.
...
Выход:
- результат замены и verification notice или сообщение об ошибке, если файл нельзя прочитать после редактирования.
...
execute(command="jupyter nbconvert --to script \"/DEMO PRES.ipynb\" --output \"/DEMO_PRES.py\"")
```
