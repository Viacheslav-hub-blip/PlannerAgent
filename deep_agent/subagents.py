...
CODING_AGENT_DESCRIPTION = (
    "Используй этого subagent (сотрудника) для задач связанных с написанием кода (рефаторинг, написание скриптов, исполнение), аналитикой"
    "(работой с dataframe, подсчет метрик, расчеты и срезы), создание файлов (перемещение, создание, редактирование)"
    "работа с jupyter файлами и так далее. Для всех этих задач стоит использовать coding agent"
)

DATA_RETRIEVAL_AGENT_NAME = "data-retrieval-agent"

DATA_RETRIEVAL_AGENT_DESCRIPTION = (
    "Используй этого subagent (сотрудника) для задач связанных к обращениям к таблицам и базам данных (выгрузка данных)"
