...
class ParsedDataQuery(BaseModel):
    ...
    table_name: str | None = Field(default=None, description="Имя Spark-таблицы или view из query без SQL-псевдонима.")
...
class ReadTableInput(BaseModel):
    ...
    Args:
        query: SQL-подобный текст запроса с именем таблицы и колонками результата.
    ...
    query: str = Field(
        description=(
            "SQL-подобный запрос. Обязательно имя Spark-таблицы или view, явный SELECT без '*', "
